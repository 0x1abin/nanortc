...
/**
 * @brief Configure a TURN relay server (RFC 5766).
 *
 * Credentials should be pre-generated by the application (e.g., coturn
 * ephemeral credentials via HMAC-SHA1). The SDK handles the TURN protocol
 * internally: Allocate (with 401 challenge-response), Refresh, CreatePermission,
 * and Send/Data indication relay.
 *
 * Call before nanortc_create_offer() / nanortc_accept_offer().
 *
 * @param rtc       Initialized RTC state.
 * @param ip        NUL-terminated TURN server IP address (IPv4 or IPv6).
 * @param port      TURN server UDP port (host byte order).
 * @param username  NUL-terminated TURN username.
 * @param password  NUL-terminated TURN credential/password.
 * @return NANORTC_OK on success.
 */
NANORTC_API int nanortc_set_turn_server(nanortc_t *rtc, const char *ip, uint16_t port,
