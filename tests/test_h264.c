...
TEST(test_h264_annex_b_null_params)
{
    ...
    ASSERT_EQ(nano_annex_b_find_nal(NULL, 10, &offset, &nal_len), NULL);
    uint8_t data[] = {0, 0, 1, 0x65};
    ASSERT_EQ(nano_annex_b_find_nal(data, 4, NULL, &nal_len), NULL);
    ASSERT_EQ(nano_annex_b_find_nal(data, 4, &offset, NULL), NULL);
}
...
TEST(test_h264_annex_b_two_nals)
{
    ...
    /* First NAL */
    const uint8_t *nal1 = nano_annex_b_find_nal(data, sizeof(data), &offset, &nal_len);
    ASSERT_TRUE(nal1 != NULL);
    ASSERT_EQ(nal1[0], 0x67);
    ASSERT_EQ(nal_len, 2); /* 0x67, 0x42 (trailing zeros stripped) */

    /* Second NAL */
    const uint8_t *nal2 = nano_annex_b_find_nal(data, sizeof(data), &offset, &nal_len);
...
TEST(test_h264_annex_b_empty)
{
    uint8_t data[] = {0};
    size_t offset = 0, nal_len = 0;
    const uint8_t *nal = nano_annex_b_find_nal(data, 0, &offset, &nal_len);
    ASSERT_EQ(nal, NULL);
}
