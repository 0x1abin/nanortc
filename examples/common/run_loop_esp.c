...
int nano_run_loop_init(nano_run_loop_t *loop, nanortc_t *rtc, const char *bind_ip, uint16_t port)
{
    if (!loop || !rtc) {
        return -1;
    }
    ...
    /* Create UDP socket */
    loop->fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (loop->fd < 0) {
    ...
    if (bind_ip) {
        size_t len = strlen(bind_ip); /* NANORTC_SAFE: API boundary */
        if (len >= sizeof(ip_str))
            len = sizeof(ip_str) - 1;
        memcpy(ip_str, bind_ip, len);
        ip_str[len] = '\0';
    }
...
static void dispatch_outputs(nano_run_loop_t *loop, uint32_t *timeout_ms)
{
    ...
    while (nanortc_poll_output(loop->rtc, &out) == NANORTC_OK) {
        ...
        case NANORTC_OUTPUT_TRANSMIT: {
            ...
            sendto(loop->fd, out.transmit.data, out.transmit.len, 0, (struct sockaddr *)&dest,
                   sizeof(dest));
            break;
        }
...
int nano_run_loop_step(nano_run_loop_t *loop)
{
    ...
    if (ret > 0 && FD_ISSET(loop->fd, &rset)) {
        ...
        int n = recvfrom(loop->fd, buf, sizeof(buf), 0, (struct sockaddr *)&from, &fromlen);
        if (n > 0) {
            nanortc_addr_t src;
