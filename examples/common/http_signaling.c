 ...
 */
static const char *http_read_response(int fd, char *resp_buf, size_t resp_buf_len, int *status_code,
                                      size_t *body_len)
{
    size_t total = 0;
    while (total < resp_buf_len - 1) {
        ...
        if (hdr_end) {
            ...
            if (!cl)
                cl = strstr(resp_buf, "content-length:");
            if (cl) {
...
static int json_extract_string(const char *json, size_t json_len, const char *key, char *out,
                               size_t out_len)
{
...
static int json_build_message(char *buf, size_t buf_len, const char *type, const char *key,
                              const char *payload)
{
    size_t wi = 0;
    int n = snprintf(buf + wi, buf_len - wi, "{\"type\":\"%s\",\"%s\":\"", type, key);
    if (n < 0 || wi + (size_t)n >= buf_len)
        return -1;
    wi += (size_t)n;
...
int http_sig_join(http_sig_t *sig, const char *host, uint16_t port)
{
    ...
    if (hlen >= sizeof(sig->host))
        hlen = sizeof(sig->host) - 1;
...
static int http_sig_send_message(http_sig_t *sig, int to_id, const char *type, const char *payload,
                                 const char *payload_key)
{
    if (!sig || sig->peer_id < 0 || !type || !payload || !payload_key)
        return -1;
    char *json = malloc(HTTP_SIG_BUF_SIZE);
    if (!json)
        return -1;
    int result = -1;
    int fd = -1;
    int jlen = json_build_message(json, HTTP_SIG_BUF_SIZE, type, payload_key, payload);
    if (jlen < 0)
        goto done;
    fd = tcp_connect(sig->host, sig->port);
    if (fd < 0)
        goto done;
    char path[64];
    if (to_id >= 0)
        snprintf(path, sizeof(path), "/send?id=%d&to=%d", sig->peer_id, to_id);
    else
        snprintf(path, sizeof(path), "/send?id=%d", sig->peer_id);
    char header[256];
    int hlen = snprintf(header, sizeof(header),
                        "POST %s HTTP/1.0\r\nHost: %s:%u\r\nContent-Type: application/json\r\n"
                        "Content-Length: %d\r\n\r\n",
                        path, sig->host, sig->port, jlen);
    if (hlen < 0 || (size_t)hlen >= sizeof(header))
        goto done;
    if (send(fd, header, (size_t)hlen, MSG_NOSIGNAL) != hlen ||
        send(fd, json, (size_t)jlen, MSG_NOSIGNAL) != jlen)
        goto done;
    char resp[512];
    int status;
    size_t blen;
...
int http_sig_send(http_sig_t *sig, const char *type, const char *payload, const char *payload_key)
{
    return http_sig_send_message(sig, -1, type, payload, payload_key);
}

int http_sig_recv_from(http_sig_t *sig, char *type_out, size_t type_len, char *payload_out,
                       size_t payload_len, int *from_out, int timeout_ms)
{
    if (!sig || sig->peer_id < 0 || !type_out || !type_len || !payload_out || !payload_len)
        return -1;
    char *resp = malloc(HTTP_SIG_BUF_SIZE);
    if (!resp)
        return -1;
    int result = -1;
    ...
    int status;
    size_t blen;
    const char *body = http_read_response(fd, resp, HTTP_SIG_BUF_SIZE, &status, &blen);
    close(fd);
...
int http_sig_recv(http_sig_t *sig, char *type_out, size_t type_len, char *payload_out,
                  size_t payload_len, int timeout_ms)
{
    return http_sig_recv_from(sig, type_out, type_len, payload_out, payload_len, NULL, timeout_ms);
}

void http_sig_leave(http_sig_t *sig)
{
    if (!sig || sig->peer_id < 0)
        return;
...
int http_sig_send_to(http_sig_t *sig, int to_id, const char *type, const char *payload,
                     const char *payload_key)
{
    if (to_id < 0)
        return -1;
    return http_sig_send_message(sig, to_id, type, payload, payload_key);
}
