                       ...
                       const char *bind_ip, uint16_t port)
{
    ...
    nano_add_local_candidate(rtc, ip_str, port);
...
static void dispatch_outputs(nano_run_loop_t *loop, uint32_t *timeout_ms)
{
    ...
    while (nano_poll_output(loop->rtc, &out) == NANO_OK) {
        switch (out.type) {
...
int nano_run_loop_step(nano_run_loop_t *loop)
{
    ...
    if (ret > 0 && FD_ISSET(loop->fd, &rset)) {
        ...
        if (n > 0) {
            ...
            nano_handle_receive(loop->rtc, now, buf, (size_t)n, &src);
        }
    } else {
        nano_handle_timeout(loop->rtc, now);
    }
