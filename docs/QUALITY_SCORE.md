...
| H.265 packetizer | `nano_h265.c` | **A** | 51 tests (Single NAL §4.4.1, FU §4.4.3 with S/E/FuType + LayerId/TID/F-bit preservation, AP §4.4.2 with LayerId/TID min + F-bit union + greedy AU packer, keyframe detection for IRAP types 16–23, PACI §4.4.4 drop, AP/FU abort transitions) | RFC 7798 Single/AP/FU packetizer + depacketizer. Hand-crafted vectors (no reference-implementation byte copies). Fuzz `fuzz_h265`: 608M+ executions clean. 90% line coverage. libdatachannel v0.22.5 interop verified (Single NAL + FU fragmentation + sprop-* on the answer). Browser smoke tracked as a follow-up validation task. |
...
| BWE | `nano_bwe.c` | **A** | 26 tests (REMB parse, byte vector, EMA smoothing, min/max clamp, event threshold, public API) | REMB parsing, EMA smoothing. Fuzz-tested (`fuzz_bwe`): 82M+ executions clean. 96% line coverage. The estimate also drives the **send-side video RTP pacer** (`NANORTC_FEATURE_VIDEO_PACING`, leaky token bucket in `nano_rtc_media.c`): it meters video egress to `BWE × NANORTC_PACING_FACTOR_PCT` so an IDR burst is spread across ~one frame interval instead of overrunning the bottleneck, with a `NANORTC_PACING_MAX_QUEUE_MS` catch-up cap bounding added latency. 5 tests in `tests/test_video_pacing.c` + admission interaction in `test_e2e.c`. |
...
| FEC send/recv | `nano_rtc_media.c` (`rtc_fec_*`) | **B** | Loopback E2E `test_e2e_video_fec_recovers_drop` (drop one media packet of a K-group → reconstruct from FEC, **zero retransmit**, all K frames delivered) + ASan-clean in the `REORDER+NACK+FEC` combo (28/28) | Phase 13 PR-3/4. Send: capture plaintext per K, emit one ULPFEC packet on a separate SSRC (media+1, PT `NANORTC_VIDEO_FEC_PT`); single-buffer reuse guard (no aliasing). Receive: PT-intercept, buffer recent media, recover the one missing member, re-inject into reorder/deliver — wire-order-independent. **NACK↔FEC coordination** (Phase 13 follow-up): the receiver skips a NACK for any SN a pending FEC protects (`rtc_fec_protects()`) so the two recovery mechanisms don't duplicate-request the same loss — verified by the FEC E2E asserting `stats_nack_sent==0` + `stats_nack_suppressed_fec>=1`. **Adaptive group size K** (`rtc_fec_effective_k()`): K tracks the smoothed TWCC loss (RFC 8888) — zero FEC overhead on a clean link, more protection (smaller K) when lossy; sender-only, receiver K-agnostic, `k`-clamped to the ring + `#error`-validated config; verified by `test_e2e_video_fec_adaptive_k_tracks_loss`. **Multi-group FEC tx ring** (`NANORTC_FEC_TX_RING`, closes TD-026 L1): a bursty IDR completing several FEC groups in one send call now emits FEC for up to N groups (not just the first), verified by `test_e2e_video_fec_multigroup_idr`. The three follow-ups: **0 confirmed findings across three 3/2-lens adversarial reviews**. ~27 KB opt-in. Remaining limits L2 (single video track) / L3 (one pending FEC) tracked as **TD-026**. **B** (not A): RED-framing + SDP negotiation (PR-2) + browser interop (PR-5) pending — functional nanortc↔nanortc only. |
