...
See [design-hardening validation](engineering/design-hardening.md).
...
| SCTP-Lite | `nano_sctp.c` | **B-** | 67 tests (codec, capacity canaries, atomic fragmentation, owned bundled delivery, iterative small-fragment reassembly, stream identity/reserved wire bits, PR-SCTP Forward TSN, gap tracking and RNG rollback) | Bounded fragmentation/reassembly, cumulative SACK, retry-limited messages and Forward TSN. Congestion control, gap-SACK fast retransmit and complete graceful shutdown remain incomplete; this is not a full RFC 9260 implementation. Fuzz-tested (`fuzz_sctp`): 63M+ executions clean. Browser + interop verified. |
| DataChannel | `nano_datachannel.c` | **B** | 18 unit tests + e2e (DCEP codec, channel mgmt, open/ack/idempotent, max channels, malformed input, all error paths) | DCEP OPEN/ACK codec, per-channel pending state with peek/commit admission, channel management and bidirectional FSM. Idempotent OPEN handling. Browser + interop verified. |
