...
| TURN client | `nano_turn.c` | **A** | 58 unit tests + real STUN/TURN discovery + strict relay-only coturn interop (handshake / DC string / channel-data burst / large payload / echo) | Full RFC 5766/8656 UDP profile: Allocate + bounded 401/438 auth + RFC 8489 retransmission + authenticated-response validation + Refresh (incl. LIFETIME=0 deallocate) + ranked, bounded CreatePermission admission + ChannelBind keyed by peer IP/port + Send/Data indication + ChannelData framing. Relay candidates join the ICE checklist and controlling/controlled data routes by the selected local candidate. The protected external workflow uses short-lived TURN REST credentials, requires real srflx+relay discovery, and then forces a relay-to-relay topology; ordinary PR/local CI excludes network tests. Fuzz-tested (`fuzz_turn`). |
| Address utils | `nano_addr.c` | **A** | 49 tests (IPv4/IPv6 parse, format, roundtrip, negative cases, global-address classification, auto-detect) | RFC 4291/5952 IPv6 parsing + formatting. Fuzz-tested (`fuzz_addr`): 70M+ executions clean. 93% line coverage. |
