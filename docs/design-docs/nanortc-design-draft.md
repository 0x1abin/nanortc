...
// ---- ICE ----
int  nano_add_local_candidate(nano_rtc_t *rtc,
                              const char *ip, uint16_t port);
int  nano_add_remote_candidate(nano_rtc_t *rtc,
...
```c
typedef enum {
    NANO_OUTPUT_TRANSMIT,   // 需要通过 UDP socket 发送的数据
    NANO_OUTPUT_EVENT,      // 应用层事件
    NANO_OUTPUT_TIMEOUT,    // 下次需要调用 handle_timeout 的时间
} nano_output_type_t;
...
│   └── nano_crypto_mbedtls.c      // 默认实现: mbedtls
│
├── examples/
...
所有数据通过 `nano_handle_receive()` 进入、通过 `nano_poll_output()` 输出，
...
通过 `nano_rtc_config_t.role` 选择角色：`NANO_ROLE_CONTROLLED` 或 `NANO_ROLE_CONTROLLING`。
...
typedef struct {
    ...
    nano_crypto_dtls_ctx_t crypto_ctx;
} nano_dtls_t;
```
...
typedef struct nano_crypto_provider {
    ...
    int  (*dtls_set_bio)(nano_crypto_dtls_ctx_t *ctx,
                         void *userdata,
                         nano_dtls_send_fn send_cb,
...
nano_rtc_config_t cfg = {
    ...
    .role = NANO_ROLE_CONTROLLED,       // ICE controlled (answerer)
    ...
    .audio_channels = 1,
    .audio_direction = NANO_DIR_SENDRECV,
    .jitter_depth_ms = 100,
#endif
...
| 发送 UDP 包 | 使用 `NANO_OUTPUT_TRANSMIT` 中的数据调用 `sendto()` |
| 接收 UDP 包 | 调用 `recvfrom()` 然后传给 `nano_handle_receive()` |
...
void nanortc_task(void *arg) {
    ...
    nano_add_local_candidate(&rtc, ip, 9999);
    ...
    for (;;) {
        ...
        while (nano_poll_output(&rtc, &out)) {
            switch (out.type) {
            case NANO_OUTPUT_TRANSMIT:
                sendto(fd, out.data, out.len, 0,
                       (struct sockaddr *)&out.dest, sizeof(out.dest));
                break;

            case NANO_OUTPUT_EVENT:
                handle_event(&rtc, &out.event);
                break;

            case NANO_OUTPUT_TIMEOUT:
    ...
    wait:;
        ...
        if (ret > 0) {
            ...
            if (n > 0) {
                nano_handle_receive(&rtc, now, buf, n, &src);
            }
        } else {
            nano_handle_timeout(&rtc, now);
        }
    }
}

static void handle_event(nano_rtc_t *rtc, nano_event_t *evt) {
    switch (evt->type) {
...
int nano_start_easy(nano_rtc_t *rtc, const nano_easy_config_t *easy_cfg);
void nano_stop_easy(nano_rtc_t *rtc);
```
...
void test_sctp_handshake(void) {
    ...
    // 客户端生成 INIT
    nano_output_t out;
    nano_handle_timeout(&client, 0);
    nano_poll_output(&client, &out);
    assert(out.type == NANO_OUTPUT_TRANSMIT);

    // 将 INIT 送给服务端
    nano_handle_receive(&server, 0, out.data, out.len, &fake_addr);

    // 服务端生成 INIT-ACK
    nano_poll_output(&server, &out);
    assert(out.type == NANO_OUTPUT_TRANSMIT);
...
3. **Sans I/O 纪律**：`src/` 目录内禁止添加 `#include <sys/socket.h>`、
   ...
   - `"nano_crypto.h"`（crypto provider 接口）
   - 不允许其他外部头文件。
