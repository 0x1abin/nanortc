...
**Coverage gap (partially closed 2026-05-17):** `tests/interop/test_interop_turn_relay_nanortc.c` lands the nanortc-as-TURN-client direction (5 cases — handshake, bidirectional DC string, ChannelData burst, large payload, echo round-trip). Each case asserts `ice.selected_type == NANORTC_ICE_CAND_RELAY` and `stats_enqueue_via_turn > 0` plus zero `stats_wrap_dropped` / zero `stats_tx_queue_full`.

The strict assertions can only run against a **dual-host / external** TURN relay — they need three distinct endpoints (nanortc, libdatachannel, relay). A single-host CI cannot provide that: over loopback, libjuice filters 127.0.0.0/8 host candidates per RFC 8838, so coturn never sees a permission-matching peer source; over a single shared non-loopback host IP (coturn in `network_mode: host`), the relay and libdc's socket share one address and coturn's hairpin back to a client on that same IP does not complete reliably. The test therefore auto-skips when the TURN host is loopback (CI default), and is run manually against a real relay (the same path Phase 5.2 verified on cellular). Bringing the strict assertions into CI would need a two-network-namespace harness and is tracked as a follow-up.

Phase 5.3 still landed a concrete fix: a perm-fan-out bug where CreatePermission family must match the allocation family per RFC 6157 §4.2 — fixed in `src/nano_rtc.c` so v6 candidates against a v4 relay (or vice versa) no longer burn perm slots with 443 responses and starve the v4 data path. This was surfaced by running the harness against a real relay during Phase 5.3 bring-up.
...
| 8 | Perm fan-out filters candidates by allocation family (Phase 5.3) | Browsers and libdc routinely trickle 4+ IPv6 host candidates first. With `NANORTC_TURN_MAX_PERMISSIONS=4` and the previous unfiltered fan-out, all 4 perm slots would be spent on v6 candidates against a v4 relay, each rejected by coturn with "443 Peer Address Family Mismatch", and the actual v4 data-path candidate never got a perm — silently breaking the relayed path. The filter is RFC 6157 §4.2 compliant (perm family must match allocation family) and is the surface fix uncovered by the nanortc-as-TURN-client interop harness. |
| 9 | nanortc-as-TURN-client strict assertions stay manual / dual-host; CI runs the test as compile + loopback-skip (Phase 5.3) | Attempted to drive the strict path in CI by feeding `NANORTC_TURN_URL` from `hostname -I`, but a single-host runner cannot satisfy the three-distinct-endpoint requirement: loopback candidates are filtered by libjuice (RFC 8838), and a single shared non-loopback host IP makes coturn relay + libdc socket collide so the hairpin never completes (observed as `libdatachannel State: Failed`, plus a teardown-path SegFault that only appears under the racy non-gdb timing). Reverted the CI injection; the test keeps its loopback auto-skip and the strict assertions are run against a real dual-host / cellular relay. A two-netns CI harness is the proper fix and is deferred. |
