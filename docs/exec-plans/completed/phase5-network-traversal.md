...
**Coverage gap (closed 2026-05-17):** `tests/interop/test_interop_turn_relay_nanortc.c` lands the nanortc-as-TURN-client direction (5 cases — handshake, bidirectional DC string, ChannelData burst, large payload, echo round-trip). Each case asserts `ice.selected_type == NANORTC_ICE_CAND_RELAY` and `stats_enqueue_via_turn > 0` plus zero `stats_wrap_dropped` / zero `stats_tx_queue_full`. CI runs the strict assertions by pointing `NANORTC_TURN_URL` at the runner's non-loopback host IP (workflow step `Detect non-loopback host IP`); on loopback URLs the test auto-skips because libjuice filters loopback host candidates per RFC 8838 and coturn then never observes a permission-matching peer source. Phase 5.3 also surfaced a perm-fan-out bug (CreatePermission family must match the allocation family per RFC 6157 §4.2) — fixed in `src/nano_rtc.c` so v6 candidates against a v4 relay (or vice versa) no longer burn perm slots with 443 responses and starve the v4 data path.
...
| 9 | CI feeds `NANORTC_TURN_URL` from `hostname -I` instead of loopback (Phase 5.3) | The nanortc-as-TURN-client direction cannot exercise its strict assertions over a loopback TURN URL: libjuice filters 127.0.0.0/8 from its host-candidate enumeration per RFC 8838, so coturn's observed peer source for libdc traffic never matches a permission target advertised in libdc's SDP. The auto-skip kicks in then. Pointing the URL at a routable runner IP keeps coturn in `network_mode: host` (works on both OrbStack and Ubuntu CI) while bypassing the loopback filter and exercising F6–F10 end-to-end. |
