...
| [Phase 5: Network Traversal](exec-plans/completed/phase5-network-traversal.md) | 2026-05-17 | 4 sessions | Full TURN + ICE srflx + per-pair STUN transaction table. Phase 5.3 closed the nanortc-as-TURN-client coverage gap with 5 strict interop cases (`test_interop_turn_relay_nanortc.c`) and surfaced an RFC 6157 §4.2 perm-family-filter bug fixed in `nano_rtc.c`. CI exercises the strict assertions by feeding `NANORTC_TURN_URL` from `hostname -I`; loopback URLs auto-skip due to libjuice RFC 8838 loopback filtering. |
