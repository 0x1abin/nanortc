 ...
 *
 * Fixed-size ring buffer indexed by seq % NANO_JITTER_SLOTS.
                ...
                size_t len, uint32_t now_ms)
{
    ...
    if (len > NANO_JITTER_SLOT_DATA_SIZE) {
        NANO_LOGW("JITTER", "packet too large for slot");
    ...
    if (delta < 0 && (uint16_t)(-delta) >= NANO_JITTER_SLOTS) {
        /* Too old, discard */
        NANO_LOGD("JITTER", "discarded stale packet");
        return NANO_ERR_NO_DATA;
    }

    /* If packet is far ahead, advance head_seq (clearing skipped slots) */
    if (delta >= (int16_t)NANO_JITTER_SLOTS) {
        /* Advance head to make room: new head = seq - NANO_JITTER_SLOTS + 1 */
        uint16_t new_head = (uint16_t)(seq - NANO_JITTER_SLOTS + 1);
        /* Clear slots that are being skipped over */
        while (jb->head_seq != new_head) {
            uint16_t idx = jb->head_seq % NANO_JITTER_SLOTS;
            jb->slots[idx].occupied = 0;
            jb->head_seq++;
        }
    }

    uint16_t idx = seq % NANO_JITTER_SLOTS;
               ...
               uint32_t *timestamp)
{
    ...
    if (!slot->occupied) {
        ...
        uint8_t found = 0;
        for (uint16_t i = 1; i < NANO_JITTER_SLOTS; i++) {
            uint16_t check_seq = (uint16_t)(jb->head_seq + i);
            uint16_t check_idx = check_seq % NANO_JITTER_SLOTS;
        ...
        idx = jb->head_seq % NANO_JITTER_SLOTS;
        slot = &jb->slots[idx];
    }
