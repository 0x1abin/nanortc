             ...
             uint8_t *buf, size_t buf_len, size_t *out_len)
{
    ...
    nano_write_u16be(buf + 2, rtp->seq);
    nano_write_u32be(buf + 4, timestamp);
    nano_write_u32be(buf + 8, rtp->ssrc);
               ...
               uint32_t *ssrc, const uint8_t **payload, size_t *payload_len)
{
    ...
    if (data[0] & 0x10) {
        ...
        uint16_t ext_len = nano_read_u16be(data + header_len + 2);
    ...
    if (seq) {
        *seq = nano_read_u16be(data + 2);
    }
    if (ts) {
        *ts = nano_read_u32be(data + 4);
    }
    if (ssrc) {
        *ssrc = nano_read_u32be(data + 8);
    }
    if (payload) {
